...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    // Subsidy is cut in half every 1575000 blocks, which will occur approximately every 5 years
    nSubsidy >>= (nHeight / 1575000); // mycoin: 1575k blocks in ~5 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 3.5 * 24 * 60 * 60; // mycoin: 3.5 days
static const int64 nTargetSpacing = 2 * 60; // mycoin: 2 minutes
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // mycoin: increase each by adding 2 to bitcoin's value.
...
CBlock* CreateNewBlock(CReserveKey& reservekey)
{
    ...
    int64 nFees = 0;
    {
        ...
        while (!mapPriority.empty())
        {
            ...
            // mycoind: Reduce the exempted free transactions to 500 bytes (from Bitcoin's 3000 bytes)
